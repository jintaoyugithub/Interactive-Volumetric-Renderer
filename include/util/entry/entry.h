...
///
struct GamepadState {
  GamepadState() { bx::memSet(m_axis, 0, sizeof(m_axis)); }

  int32_t m_axis[entry::GamepadAxis::Count];
};

///
bool processEvents(uint32_t &_width, uint32_t &_height, uint32_t &_debug,
                   uint32_t &_reset, MouseState *_mouse = NULL);
...
///
struct WindowState {
  WindowState() : m_width(0), m_height(0), m_nwh(NULL) {
...
///
bool processWindowEvents(WindowState &_state, uint32_t &_debug,
                         uint32_t &_reset);
...
public:
  ...
  ///
  virtual void init(int32_t _argc, const char *const *_argv, uint32_t _width,
                    uint32_t _height) = 0;
